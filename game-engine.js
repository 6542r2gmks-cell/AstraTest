...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!window.Matter || !window.GameRuntime) return;
    const clock = GameRuntime.clock = FC.createClock(() => String(state.phase));
    const {setTimeout, setInterval, clearTimeout, clearInterval, requestAnimationFrame, cancelAnimationFrame} = clock;
    const localStorage = GameRuntime.store;
    ...
    GameRuntime.physics = physicsRunner;
    ...
    function syncPhysicsForPhase(phaseId) {
        ...
        physicsRunner.enabled = physicsDomSyncEnabled && !GameRuntime.paused;
    }
    Runner.run(physicsRunner, physicsEngine);
    physicsRunner.enabled=false;
    ...
    function saveProgress() {
        ...
        const progress = {
            ...
            mapUnlocked: state.mapUnlocked,
            gasProduct: state.gasProduct,
        ...
        pendingResume = {phase:state.phase, product:state.product};
        window.dispatchEvent(new CustomEvent('fc:progress', {detail:progress}));
    }
    ...
    function onTap(el, handler) {
        ...
        if (!el.getAttribute('aria-label')) el.setAttribute('aria-label', el.textContent.trim() || 'Interact with molecule');
        ...
        el.addEventListener('keydown', function(e) {
            ...
            e.preventDefault(); handler.call(this,e);
        });
    }

    /** Track GA4 event safely */
    function track() { /* No student analytics or session recording in v2. */ }
    ...
    function markUnitComplete(unitId) {
        if (!V804_UNITS.some(unit=>unit.id===unitId)) return;
        const isNew = !state.completedUnits.includes(unitId);
        if(isNew) state.completedUnits.push(unitId);
        saveProgress();
        updateV804Tracker(); updateMapUI();
        if(isNew) window.dispatchEvent(new CustomEvent('fc:unit-complete',{detail:{unitId}}));
    }
    ...
    const funFacts = {
        ...
        alky: {
            emoji: '⛽',
            text: "Alkylate is a clean, high-octane gasoline blending component. It has very low sulfur, no olefins, and no aromatics. This makes it burn smoothly and reliably, making it a premium blendstock!"
        },
        reformer: {
            emoji: '🔬',
    ...
    function renderFunFactOverlay(fact, options = {}) {
        ...
        overlay.setAttribute('aria-label', options.label || 'Refinery fact');
        const focusTarget = options.mode==='browser' ? nextBtn : getEl('fun-fact-tap');
    ...
    function showFunFact(factKey, callback) {
        ...
        function dismiss() {
            ...
            overlay.classList.remove('active');
            getEl('fun-fact-tap').onclick=null;
        ...
        // Facts are learner-paced; reading is never auto-dismissed.
        const continueButton = getEl('fun-fact-tap');
        if(continueButton) continueButton.onclick = dismiss;
    }
...
function showPhase(phaseId, options = {}) {
    ...
    state.phase = String(phaseId);
    if(phaseId==='finale') state.mapUnlocked=true;
    ...
    phaseTransitionTimeout = setTimeout(() => {
        ...
        window.dispatchEvent(new CustomEvent('fc:phase',{detail:{phase:String(phaseId),product:state.product}}));
        phaseTransitionTimeout = null;
...
function mapJump(phaseId, defaultProduct) {
    ...
    // The destination transition releases the navigation lock on activation.
    ...
    function startGameplay() {
        ...
        function spawnTar(preSettled = false) {
            ...
            tar.addEventListener('pointerdown', function (e) {
                ...
                e.preventDefault();
                isDragging = true;
                try { tar.setPointerCapture(e.pointerId); } catch {}
                ...
                function onUp(e) {
                    isDragging = false;
                    try { tar.releasePointerCapture(e.pointerId); } catch {}
                    tar.removeEventListener('pointermove', onMove);
                    tar.removeEventListener('pointerup', onUp);
                    tar.removeEventListener('pointercancel', onCancel);
                ...
                function onCancel(e) {
                    ...
                    tar.style.left=currentLeft+'%'; tar.style.top=currentTop+'%';
                }
                tar.addEventListener('pointercancel',onCancel);
...
`;
    ...
    function finishGame() {
        ...
        if (aromatics === 3) {
            ...
            finishMsg = `⚠️ Heavy cracking — most of the feed was destroyed. Try less heat on the next run.`;
        } else {
  ...
  `;
    ...
    function spawnSpentCat(startLeft, startTop) {
        ...
        cat.addEventListener('pointerdown', (e) => {
            ...
            try { cat.setPointerCapture(e.pointerId); } catch {}
            ...
            const onUp = (ev) => {
                ...
                cat.removeEventListener('pointercancel', onCancel);
            ...
            function onCancel() {
                ...
                dropZone.classList.remove('fcc-drop-zone--hot');
                cat.classList.remove('fcc-dragging');
                cat.style.position='absolute'; cat.style.left=homeLeft; cat.style.top=homeTop;
            }
            cat.addEventListener('pointercancel',onCancel);
...
function showJetIntroCard(onStart) {
    ...
    document.body.appendChild(overlay);
    overlay.addEventListener('click', (event) => {
        if (event.target.id === 'jet-intro-start') {
            overlay.remove();
            if(onStart) onStart();
        }
    });
...
function setupMinigame() {
    ...
    if (state.product === 'gasoline' || state.product === 'lpg' || state.product === 'naphtha') {
        ...
        showJetIntroCard(() => startJetCOAGame(container, labCheck));
...
function setSruStatus(statusKey, statusText, options = {}) {
    ...
    const now = (typeof performance !== 'undefined' && typeof performance.now === 'function') ? clock.now() : Date.now();
...
========================================= */
function loadGame() {
    const progress = localStorage.read();
    pendingResume=null;
    if(progress){
        state.completedUnits=progress.completedUnits;
        state.gasGradesCompleted=progress.gasGradesCompleted;
        state.gasProduct=progress.gasProduct;
        state.product=progress.product;
        state.mapUnlocked=progress.mapUnlocked;
        ...
        if(state.mapUnlocked) getEl('game-map')?.classList.remove('hidden');
        pendingResume={phase:progress.phase,product:progress.product};
    }
    updateMapUI();
    const printBtn=getEl('v804-print-btn');
    if(printBtn) printBtn.onclick=()=>{
        if(!V804_UNITS.every(u=>state.completedUnits.includes(u.id))) return;
        const win=window.open('certificate.html','_blank');
        if(!win) window.location.assign('certificate.html');
    };
...
function initializePumpSwapSession() {
    ...
    pumpState.lastTickAt = clock.now();
    pumpGameLoop = registerPhaseInterval('pump-swap', () => pumpSystemTick(clock.now()), 250);
}
...
function evaluatePumpRules() {
    ...
    if (!pumpState.A.motor && !pumpState.A.suction && !pumpState.A.discharge &&
        ...
        endPumpGame(true, 'success', 'Pump swap complete', 'Pump B is carrying the feed. This completes the game sequence; real maintenance requires the full site isolation procedure.');
    }
}

function pumpSystemTick(now = clock.now()) {
...
========================================= */
window.Game = {
    getProgress: () => ({...FC.normalize({...state}),activePhase:String(state.phase)}),
    getUnits: () => V804_UNITS.map(u=>({...u})),
    reloadProgress: loadGame,
    returnToStart: () => showPhase('0',{skipSave:true}),
    showPhase,
    startOrResume,
    openFactsBrowser,
    nextFact,
